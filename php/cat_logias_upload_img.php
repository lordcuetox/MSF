...
if (isset($_POST['xAccion2'])) {
    if ($_POST["xAccion2"] == "upload") {
        ...
        $cve_logia = isset($_POST['xCveLogia']) ? $_POST['xCveLogia'] : 0;
        $target_dir = "../img/logias/";
        ...
        $new_name_file = $cve_logia . $extension;
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                ...
                $sql = "UPDATE logias SET foto = '" . (substr($target_file, 3, strlen($target_file))) . "' WHERE cve_logia = $cve_logia";
        ...
        header('Location:cat_logias.php');
        return;
    }
}
?>
<div class="modal-header">
    ...
    <h4 class="modal-title">Logias | Subir imagen</h4>
</div>
    ...
    <div class="te">
        <form role="form" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data" id="frmUpload" name="frmUpload">
            <div class="form-group">
                <input type="hidden" id="xCveLogia" name="xCveLogia" value="<?php echo($_GET["xCveLogia"]); ?>" />
